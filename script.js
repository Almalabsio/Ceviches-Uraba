...
// ============================================
// PAGE & NAVIGATION (Optimized Loading)
// ============================================
// Hide preloader more aggressively
const hidePreloader = () => {
    if (preloader && !preloader.classList.contains('hide')) {
        preloader.classList.add('hide');
        // Let AOS know it can start
...
// Start trying to hide as soon as possible
if (document.readyState === 'complete') {
    hidePreloader();
} else {
    // Hide after a fixed delay even if not all images (1.5s total)
    setTimeout(hidePreloader, 1500);
    // Or hide as soon as everything IS loaded
...
// ============================================
// HERO VIDEO CAROUSEL (Optimized for Smoothness)
// ============================================
...
const crossfade = () => {
    ...
    // Play next video while still hidden
    next.play().then(() => {
        // Start fade in
        next.classList.add('active');

        // After transition completes (matching CSS 2.5s)
        setTimeout(() => {
        ...
        isTransitioning = false;
        // Fallback: just skip to next if play fails
        currentVideo = nextIndex;
    });
};
...
if (heroVideos.length > 1) {
    ...
    heroVideos.forEach((video, index) => {
        // Use 'timeupdate' to trigger crossfade slightly before end for seamless loop
        video.addEventListener('timeupdate', function () {
            if (this.classList.contains('active') && !isTransitioning) {
                const remaining = this.duration - this.currentTime;
                // Start crossfade 1 second before the current video ends
