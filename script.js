...
REGLAS DE BLOQUEO (aplícalas con tu misma calidez, nunca de forma brusca):
— Si el cliente NO ha dado su nombre Y su número de WhatsApp: puedes responder preguntas del menú o precios, pero SIEMPRE termina tu mensaje pidiéndolos de forma cálida y creativa. Varía la forma de pedirlos en cada mensaje. No avances al proceso de cierre sin ellos.
— Si el cliente ya eligió su pedido pero no ha confirmado la hora de llegada o recogida: pregúntala antes de cerrar. Sin hora no hay cierre.
— Antes de generar el bloque [[PEDIDO]] verifica internamente que tienes los 6 datos: Nombre, Teléfono, Producto, Porciones, Modo y Hora. Si falta cualquiera, pídelo primero con amabilidad y sigue la conversación hasta obtenerlo.
— PROHIBIDO INVENTAR INFORMACIÓN: Jamás menciones pedidos anteriores, platos favoritos ni historial de ningún tipo a menos que hayas recibido explícitamente un bloque "CONTEXTO DE CLIENTE RECURRENTE" en esta conversación. Si solo tienes el nombre pero no el teléfono, trátalo como cliente nuevo. El historial real solo llega después de que el cliente da su número de WhatsApp — hasta ese momento no sabes nada de él.
...
const callAPI = async (messages) => {
    ...
    // Proxy seguro — la key vive en Apps Script, nunca en el cliente
    const response = await fetch(SHEET_URL, {
        method: "POST",
        headers: { "Content-Type": "text/plain" },
        body: JSON.stringify({ accion: "aiChat", messages: msgsConFecha })
    });
    if (!response.ok) throw new Error("API " + response.status);
    const data = await response.json();
    if (!data.ok) throw new Error(data.msg || "Sin respuesta de IA");
    const raw = data.text;
